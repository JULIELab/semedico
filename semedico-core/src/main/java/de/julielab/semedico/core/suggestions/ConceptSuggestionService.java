 ...
 * @author faessler
 */
public class ConceptSuggestionService implements IConceptSuggestionService {
    ...
    private final String[] filterIndices;
    ...
    public ConceptSuggestionService(Logger logger, IConceptService termService,
                                    ...
                                    @Symbol(SemedicoSymbolConstants.SUGGESTION_FILTER_INDICES) String filterIndices,
        ...
        this.filterIndices = filterIndices.split(",");
    }

    /*
     * (non-Javadoc)
     *
     * @see de.julielab.semedico.suggestions.IConceptSuggestionService#
        ...
        try {
            ...
            if (activated) {
                Collection<? extends FacetTermSuggestionStream> defaultSuggestions = getDefaultSuggestions(termFragment,
                        facets);
                // TODO
                LegacySemedicoSearchResult suggestionResult = null;//(LegacySemedicoSearchResult) searchService.doSuggestionSearch(termFragment, facets).get();
                    ...
                    if (!suggestionResult.suggestions.isEmpty()) {
                        ...
                        if (termFragment.contains(" ")) {
                            ...
                            while (suggestionResult.suggestions.isEmpty() && beginIndex < split.length) {
                                ...
                                // TODO
                                suggestionResult = null;//((LegacySemedicoSearchResult)searchService.doSuggestionSearch(suffix, facets).get());
...
//		return null;
    ...
    protected void addSuggestionsForDatabaseTermsCompletionStrategy() {
        ...
        try {
            Set<String> termIdsInIndex = Collections.emptySet();
            if (!indexAll) {
                termIdsInIndex = getConceptIdsInSuggestionFacets();
            }
            final Set<String> finalIndexTerms = termIdsInIndex;
            ...
            Iterator<Map<String, Object>> docIt = new Iterator<Map<String, Object>>() {
                ...
                public boolean hasNext() {
                    ...
                    do {
                        ...
                        if (!indexAll)
                            termOccuresInIndex = finalIndexTerms.contains(nextTerm.getId());
                ...
                public Map<String, Object> next() {
                    ...
                    suggestionMap.put(IConceptSuggestionService.SUGGESTION_TEXT.INPUT, Lists.newArrayList(suggestion));
                    suggestionMap.put(IConceptSuggestionService.SUGGESTION_TEXT.WEIGHT, 100 / suggestion.length());
                    suggestionContextMap.put(IConceptSuggestionService.SUGGESTION_TEXT.INPUT,
    ...
    private Set<String> getConceptIdsInSuggestionFacets() throws InterruptedException, ExecutionException {
        Set<String> termIdsInIndex = new HashSet<>();
        final List<Facet> suggestionFacets = facetService.getSuggestionFacets();
        log.info(
        ...
        for (String filterIndex : filterIndices) {
            ...
            final List<AggregationRequest> conceptRequests = suggestionFacets.stream()
                    ...
                    .collect(toList());
            // Add all the aggregation requests to the query
            conceptRequests.forEach(elasticSearchQuery::putAggregationRequest);
            // Execute the query
            final FieldTermsRetrievalResult conceptIds = searchService.search(elasticSearchQuery, EnumSet.of(SearchService.SearchOption.NO_HITS), ResultCollectors.getFieldTermsCollector("allfacetterms", conceptRequests.toArray(new AggregationRequest[0]))).get();
            // Put all the terms into the set
            termIdsInIndex.addAll(conceptIds.getFieldTerms().entrySet().stream().flatMap(Map.Entry::getValue).map(fti -> fti.term).map(String.class::cast).collect(toList()));
            log.info("Retrieved {} concept IDs from suggestion facet fields in index {}.", termIdsInIndex.size(), filterIndex);
        }
        return termIdsInIndex;
    }
