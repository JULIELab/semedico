...
import de.julielab.semedico.core.facets.FacetGroupLabels.Type;

/**
 * A class to assembly all <code>Facet</code> objects belonging to the same display group.
 * <p>
...
public class FacetGroup<T extends Facet> extends ArrayList<T> implements Comparable<FacetGroup<T>> {
	...
	private final Multimap<String, T> facetsBySourceName;
	...
	public Collection<T> getTaxonomicalElements() {
		Collection<T> facets = new HashSet<>();
		for (T facet : this)
	...
	public void setType(Type facetGroupType) {
