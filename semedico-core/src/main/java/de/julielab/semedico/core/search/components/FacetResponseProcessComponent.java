...
import de.julielab.scicopia.core.elasticsearch.legacy.ISignificantTermsAggregationUnit;
import de.julielab.scicopia.core.elasticsearch.legacy.SignificantTermsAggregationResult;
	...
	public boolean processSearch(SearchCarrier searchCarrier) {
		...
		try {
			...
			if (null == searchResponse) {
				throw new IllegalArgumentException("The Elasticsearch response must not be null, but it is.");
			}
			if (null == uiState) {
			...
			// Extract the facet counts from Elasticsearch's response and store them to
			// the user's interface state object.
	...
	private void storeHitFacetTermLabels(ISearchServerResponse serverResponse, SearchServerCommand serverCmd,
		...
		for (IFacetField facetField : serverResponse.getFacetFields()) {
			...
			for (final UIFacet uiFacet : uiFacetsWithSrcName) {
				...
				// serve a particular Semedico facet. This could be the field for
				// total facet counts, for example.
				if (uiFacet == null) {
					continue;
				}
	...
	private void createLabels(LabelStore labelStore, Map<String, PairStream<Concept, Long>> termCountsByFacetId,
		...
		for (String facetId : termCountsByFacetId.keySet()) {
			...
			log.trace("Facet {} (ID: {}) received {} labels with >0 count.",
					facet.getName(), facet.getId(), termCount);
			...
			if (!uiFacet.isFlat() && !uiFacet.allDBRootsLoaded())
				log.debug("Loading facet roots of facet {} (ID: {}) as given by the following list of term IDs: {}",
						uiFacet.getName(), uiFacet.getId(), hierarchicalTermIdsForThisFacet);
