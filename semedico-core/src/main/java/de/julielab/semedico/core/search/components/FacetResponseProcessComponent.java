...
public class FacetResponseProcessComponent extends AbstractSearchComponent {
	...
	public boolean processSearch(SearchCarrier searchCarrier) {
		...
		try {
			...
			// Extract the facet counts from the search server response and store them to
			// the
			...
			log.error("Could not load facet concepts", e);
		} catch (ConceptCreationException e) {
			log.error("Could not create facet concepts", e);
		}
        return false;
	}
	...
	private void storeTotalFacetCounts(SearchServerRequest serverCmd, ISearchServerResponse searchServerResponse, LabelStore labelStore) {
		...
		for (AggregationRequest aggCmd : serverCmd.aggregationRequests.values()) {
			// TODO repair
			IAggregationResult aggResult = null;// searchServerResponse.getAggregationResult(aggCmd);
	...
	private void storeHitFacetTermLabels(ISearchServerResponse serverResponse, SearchServerRequest serverCmd,
			AbstractUserInterfaceState uiState) throws ConceptLoadingException, ConceptCreationException {
		...
		for (AggregationRequest aggCmd : serverCmd.aggregationRequests.values()) {
			...
			for (final UIFacet uiFacet : uiFacetsWithSrcName) {
				...
				// TODO repair
				final TermsAggregationResult termsAggResult = null;//(TermsAggregationResult) serverResponse.getAggregationResult(aggCmd);
				...
				if (uiFacet.getSource().isStringTermSource())
					log.trace(
							"Facet {} with ID {} has a string facetSource, retrieved index terms are used verbatim as string terms.",
							uiFacet.getName(), uiFacet.getId());
				else if (uiFacet.getSource().isDatabaseTermSource())
					log.trace(
							"Facet {} with ID {} has a database term facetSource, retrieved index terms are used as database term identifiers.",
							uiFacet.getName(), uiFacet.getId());
				else
					log.warn("Facet {} with ID {} has an unhandeled facetSource type: {}", uiFacet.getName(),
		...
		if (null != serverCmd.aggregationRequests
			...
			// TODO repair // remove
			SignificantTermsAggregationResult sigAggRes = null;//(SignificantTermsAggregationResult) serverResponse
		...
		// conceptService.getTermCountsForAuthorFacets(authorCounts,
		// searchState.getId());
	...
	private void createLabels(LabelStore labelStore, Map<String, PairStream<Concept, Long>> termCountsByFacetId,
			Map<Facet, UIFacet> uiFacets) throws ConceptLoadingException, ConceptCreationException {
