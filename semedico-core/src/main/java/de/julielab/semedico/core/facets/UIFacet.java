...
public class UIFacet extends Facet {
    ...
    // Whether this uiFacet shall not be displayed to the user in
    ...
    /**
     * Indicates if this facet is forced to flat, frequency ordered concept counts.
     *
    ...
    /**
     * The list of concepts on the path from the root (inclusive) to the currently selected concept (inclusive) of this facet.
    ...
    public UIFacet(Logger logger, Facet originalFacet) {
        ...
        this.logger = logger;
        this.viewMode = facetSource != null && facetSource.isHierarchic() ? FacetViewMode.HIERARCHIC : FacetViewMode.FLAT;
    ...
    /**
     * aka flat, perhaps 'list' is a better concept here
     */
     ...
     * <ul>
     * <li>If the facet is not drilled down, i.e. the user did not select any concept of this facet and did not enter a
     * search concept associated with the facet, the facet root IDs are returned.
     * <li>If the facet is drilled down, i.e. the user has been viewing successors of a root concept, the children of the
     * last clicked-on concept - i.e. the root of the currently viewed subtree - are returned.</code>.
     * </ul>
     * </p>
     *
     ...
     * @throws IncompatibleStructureException When this method is invoked on a facet instance which has a flat structure.
     */
    ...
    /**
     * Appends <code>concept</code> to the current path from a root to the currently selected concept for this configuration's
     * facet.
     *
     * @param concept The concept to append to the current path.
     */
    public void appendNodeToCurrentPath(Concept concept) {
        forcedToFlatFacetCounts = false;
        currentPath.appendNode(concept);
    }
    ...
    public boolean containsCurrentPathNode(Concept concept) {
        return currentPath.containsNode(concept);
    }

    /**
     * Returns the last - i.e. the deepest - element of the current root-to-concept path of this
     * <code>FacetConfiguration</code>.
     *
     * @return The last element on the current root-to-concept path or <code>null</code> if the path is empty.
     */
     ...
     * <li>the view mode will be reset to the natural structure of the underlying facet's facetSource (hierarchical vs.
     * flat).</li>
     * </ul>
     */
    public void reset() {
        ...
        collapsed = false;
        expanded = false;
        this.viewMode = facetSource.isHierarchic() ? FacetViewMode.HIERARCHIC : FacetViewMode.FLAT;
        clearCurrentPath();
        displayGroup.reset();
    }
    ...
    /**
     * @param forcedToFlatFacetCounts the forcedToFlatFacetCounts to set
     */
