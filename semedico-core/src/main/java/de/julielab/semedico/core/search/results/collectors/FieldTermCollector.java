...
public class FieldTermCollector extends SearchResultCollector<SemedicoESSearchCarrier, FieldTermsRetrievalResult> {
    ...
    public FieldTermsRetrievalResult collectResult(SemedicoESSearchCarrier semCarrier, int responseIndex) {
        final PrerequisiteChecker checker = PrerequisiteChecker.checkThat()
                .notEmpty(semCarrier.getSearchResponses())
                .notNull(semCarrier.getSearchResponse(0))
                .notNull(semCarrier.getServerRequest(responseIndex).aggregationRequests)
                .notNull(((ElasticServerResponse)semCarrier.getSearchResponse(0)).getResponse())
                .withNames("Search Server Responses", "Search Server Response at index 0", "Aggregation Requests", "Elasticsearch Response");
        Map<String, AggregationRequest> aggregationRequests = semCarrier.getServerRequest(responseIndex).aggregationRequests;
        aggregationNames.forEach(n -> checker
                .notNull((Supplier<AggregationRequest>) () -> aggregationRequests.get(n))
                .withNames("Aggregation with name " + n));
        checker.execute();
        ...
        ISearchServerResponse serverResponse = semCarrier.getSearchResponse(0);
