...
public class ParseTree {
     ...
     * @param errors Object containing error messages.
     */
    ...
    /**
     * Creates a parse tree with a single node that contains the given text.
     *
     * @param phrase The text to search for. My be one or several words. In the latter case, the text will be handled as a phrase.
     * @return A parse tree expressing the input phrase.
     */
    public static ParseTree ofPhrase(String phrase) {
        final QueryToken qt = new QueryToken(0, phrase.length());
        return new ParseTree(new TextNode(phrase, qt), new ParseErrors());
    }

    /**
     * Creates a parse tree returning a conjunction of the whitespace-separated words in the input text.
     *
     ...
     *
     * @param texts Must map oldText:substitution.
     */
     ...
     *
     * @param text  Text node to expand.
     * @param texts Content for the new nodes.
     */
     ...
     *
     * @param existingNode The existing node that the new node is added to as a sibling.
     * @param newNode      The new node that is added as a sibling to the existing node.
     * @param operator     The operator type for the inserted parent node of both the
     ...
     *
     * @param text Text to find in the tree.
     ...
     *
     * @param id Node id to find in the tree.
    ...
    private long mapTree(Node node, BranchNode parent, long id) {
        ...
        if (node.isLeaf() && node.getClass() != TextNode.class) {
            ...
            // node to a text node.
            // HERE WOULD COMING FIXING CODE (used to be event handling).
     ...
     * @param termList An empty list of terms to fill.
    ...
    private void addSubtreeToTermList(Node node, List<IConcept> termList) {
        if (node instanceof BranchNode) {
            ...
            termList.addAll(textNode.getConcepts());
        }
        ...
        // } else if (node.getClass().equals(TextNode.class)) {
        // TextNode textNode = (TextNode) node;
        // for (IConcept term : textNode.getConcepts()) {
        // termList.add(term);
        // }
        // }
    }
    ...
    private void selectTerm(Node node, IConcept term) {
        if (!foundTerm) {
            if (node.getClass().equals(BinaryNode.class)) {
                ...
                for (IConcept existingTerm : textNode.getConcepts()) {
                ...
                for (IConcept termInList : existingTerms) {
                    if (termInList.equals(term)) {
                        ...
                        textNode.getQueryToken().setConceptList(newTermList);
                        foundTerm = true;
                    }
                }
            }
    ...
    public List<TextNode> getUnambigousTextNodes() {
        ...
        for (Node node : textMap.values()) {
            ...
            if (null != textNode && textNode.getConcepts().size() == 1) {
                textNodes.add((TextNode) node);
            }
        }
        return textNodes;
    ...
    /**
     * Traverses the tree given by <code>subtreeRoot</code> and adds those nodes to <code>collector</code> that are
     * of the given class.
     *
     * @param subtreeRoot Root of a parse (sub)tree
     * @param cls         The Node class that determines which nodes are collected.
     * @param collector   A collection to collect the nodes of class <code>cls</code> in that were found in the
     *                    tree given by <code>root</code>.
     * @return The collector for convenience.
     */
    public <T extends Collection<Node>> T getNodes(Node subtreeRoot, Class<? extends Node> cls, T collector) {
    ...
    public Stream<Long> getPreOrderNodeIds() {
        return traversePreOrder().stream().map(Node::getId);
    }

    public Stream<Long> getPreOrderNodeIds(final boolean descendIntoEvents) {
        return traversePreOrder(descendIntoEvents).stream().map(Node::getId);
    }
    ...
    public boolean isAll(Class<? extends IConcept> conceptClass) {
        // Try to find any concept that is NOT of the given concept class
        Optional<? extends IConcept> otherNodes = getNodes(root, TextNode.class, new ArrayList<>()).stream().
                map(TextNode.class::cast).
                map(TextNode::getConcepts).
                flatMap(Collection::stream).
                filter(concept -> !conceptClass.equals(concept.getClass())).
                findAny();
        return !otherNodes.isPresent();
    }

    public enum Serialization {
        NODE_TEXT, CONCEPT_IDS, NODE_IDS, CONCEPT_NAME_TYPE
    }
}
