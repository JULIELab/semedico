...
//	}
	...
	public String toString(Serialization serializationType) {
		...
		String id = serializationType == Serialization.NODE_IDS ? String.valueOf(this.id) + " " : "";
		for (Node child : children)
			childStrings.add(child.toString(serializationType));
		String nodeTypeName = nodeType.name();
		if (serializationType == Serialization.CONCEPT_NAME_TYPE)
			// Type "operator"
			nodeTypeName += "[OP]";
		if (nodeType != NodeType.NOT)
			return "(" + StringUtils.join(childStrings, " " + id + nodeTypeName + " ") + ")";
		else
			return "(" + id + nodeTypeName + " " + childStrings.get(0) + ")";
	}
