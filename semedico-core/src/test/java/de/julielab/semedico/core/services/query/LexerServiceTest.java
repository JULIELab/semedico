...
public class LexerServiceTest {
	...
	@Test
	public void testTopicTags() throws IOException {
		List<QueryToken> lex = lex("#medicine");
        assertThat(lex).hasSize(1);
        assertThat(lex.get(0).getType()).isEqualTo(QueryTokenizerImpl.HASHTAG);
        assertThat(lex.get(0).getInputTokenType()).isEqualTo(TokenType.TOPIC_TAG);
	}
