...
public class QueryPanel {
	...
	public IConcept getFirstTermOfParseNode() {
		...
		if (null == node)
			return null;
		logger.debug("Current parse tree node: " + node.toString(SERIALIZATION.TEXT));
	...
	public void onDrillUp(Integer nodeIdToDrillUp, int pathItemIndex) throws Exception {
		...
		Concept searchTerm = (Concept) textNode.getTerms().get(0);
		...
		for (TextNode unambigousQueryTerm : parseTree.getUnambigousTextNodes()) {
			IConcept term = unambigousQueryTerm.getTerms().get(0);
		...
		textNode.setTerms(parentCollection);
	...
	public List<IConcept> getNodeConcepts() {
		...
		if (null != node) {
			List<IConcept> mappedQueryTerms = new ArrayList<>(node.getTerms());
	...
	public IConcept getFirstTermOfEventArgument() {
		if (eventArgItem.isConceptNode()) {
			...
			return textNode.getTerms().get(0);
		}
		return null;
	}
