define(
        ...
        function($) {
            ...
            // This overrides a security function of jQueryUI which by default
            // doesn't allow HTML in the title to avoid scripting
            // vulnerabilities.
            ...
            /**
             * An empty, global variable to hold an instance of the
             * DisambiguationManager.
             */
            var disambiguationManager = {}
            /**
             * Called from the Tapestry DisambiguationDialog component. Sets up
             * the DisambiguationManager that then handles all things associated
             * with disambiguation, from assigning the correct CSS class to
             * showing the disambiguation dialog and the disambiguation itself.
             */
            var init = function(parameters) {
                console.log("disambiguation dialog is created")
            ...
            /**
             * This object is instantiated at first by the Tapestry initializer
             * call issued by DisambiguationDialog component.
             */
            function DisambiguationManager(elementId, disambiguationZoneId) {
                ...
                function setupDisambiguationDialog() {
                    ...
                    // explicitly close the dialog when clicking outside of the
                    // dialog
                ...
                this.openDisambiguationDialog = function() {
                    // If the dialog is still visible (e.g. is currently in its
                    // hide
                ...
                // setup token DOM objects (the li elements in of the token
                // list)
                // this method is also called from autocomplete.js which leads
                // to (currently
            ...
            /**
             * For each ambiguous token, one instance of this class is created
             * by the DisambiguationManager. This class handles the ajax
             * communication with the server to fill the disambiguation dialog
             * with the correct HTML contents. This class uses the
             * DisambiguationManager to to open the dialog and update the tokens
             * when a token is disambiguated.
             */
            function AmbiguousQueryToken(token, domElement, tokenindex, dialogElementId, disambiguationZoneId) {
                ...
                var init = function() {
                    domElement.mouseenter(function() {
                        // At first, remove all event listeners for the zone
                        // update.
                        // Necessary for the case when we have multiple
                        // ambiguous tokens
                        // because we only have a single dialog. When all
                        // ambiguous tokens
                        // listen for the event, they will all get triggered as
                        // soon as we
                        // disambiguate to a token and then all get replaced by
                        // the selected
                        // concept.
                        $('#' + disambiguationZoneId).off("t5:zone:did-update");
                        // No attach an event listener that is fired when the
                        // disambiguation
                        // dialog zone has been updated to show the possible
                        // options for the
                        ...
                        if (!disambiguationManager.dialogIsOpen()) {
                            // first, set the title and position of the dialog
                            // to the token
                ...
                var disambiguate = function() {
                    ...
                    // this triggers the Tapestry-provided zone refresh
                    // directly, so we
                    ...
                    // // // insert response into disambiguation dialog
                    // // var responseHTML = data.zones[disambiguationZoneId];
                    // // $('#' + dialogElementId).html(responseHTML);
                    // // Change title of disambiguation dialog
                    // $('#' + dialogElementId).dialog("option", {
                    // title : "Disambiguation of <span
                    ...
                    // $('#' + dialogElementId + '
                    // .disambiguationTerm').click(function() {
                    // // what happens if one of the disambiguation options is
                    // clicked
                    // // on...
                    // $('#' + dialogElementId).dialog("close");
                    // // var selectedConceptName = $(this.innerHTML).text();
                    // var selectedConceptId = this.getAttribute('conceptid');
                    // $.getJSON(token.getConceptTokensLink + "?q=" +
                    // selectedConceptId,
                    // function(response) {
                    // // Replace token in searchbar,
                    // disambiguationManager.replaceTokens(token, response[0]);
                    // // after disambiguation we have to set the correct
                    // tooltips;
                    // // however, autocomplete.js defines anyway and it is
                    // repeated
                    // // for
                    // // each token because we add all tokens anew. Best would
                    // be to
                    // // run it
                    // // once, of course.
                ...
                function setupDisambiguationHandler(event) {
                    ...
                    $('#' + dialogElementId + ' .disambiguationTerm').click(function() {
                        ...
                        $.getJSON(token.getConceptTokensLink + "?q=" + selectedConceptId, function(response) {
                            ...
                            // repeated
                            // for
                            // each token because we add all tokens anew. Best
                            // would be to
                            // run it
                            // once, of course.
                            // disambiguationManager.setupTokens();
                                ...
                                .tooltip(
                                        {
                                            position : {
                                                ...
                                                using : function(position, feedback) {
                                                    $(this).css(position);
                                                    // only display the callout
                                                    // if the tooltip is
                                                    // actually positioned top
                                                    if (feedback.vertical == "top")
                                                        $("<div>").addClass("arrow callout-left black small").addClass(
                                                                feedback.vertical).addClass(feedback.horizontal)
                                                                .appendTo(this);
                                                }
                                            },
                                            content : function() {
            ...
            return {
                "init":init,
                "setupTokens" : disambiguationManager.setupTokens
            }
        })
