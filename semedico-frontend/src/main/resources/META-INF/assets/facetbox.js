...
/*
 * The method refreshListeners is called when the content of a facet has
 * changed. It updates the event listeners accordingly.
 */
FacetBox.prototype.refreshListeners = function() {
    ...
    if (this.expanded) {
        ...
        if (this.filterField) {
            // Event.observe(this.filterField, "keypress",
            // this.onKeyPress.bindAsEventListener(this));
            Event.observe(this.filterField, "keydown", this.onKeyDown.bindAsEventListener(this));
            Event.observe(this.filterField, "click", this.selectAllFilterText.bindAsEventListener(this.filterField));
            if (this.filterField.value == "type to filter") {
                $(this.filterField).setStyle({
                    "color" : "grey"
                });
                Event.observe(this.filterField, "focus", this.filterField.clear.bindAsEventListener(this.filterField));
                Event.observe(this.filterField, "blur", this.resetFilter.bindAsEventListener(this.filterField));
            } else {
...
/*
 * Updates the facet box with the JSON code supplied as argument content see
 * http://www.json.org/
 */
FacetBox.prototype.updateBox = function(content) {
    if (content == "") {
        ...
        // WARNING this is a jQuery statement to remove the jQuery UI tooltips
        // potentially currently shown on facet terms we are about to replace.
        // Then, the tooltip will never disappear, so we destroy them first.
        $j(Semedico.facetTooltipSelector).tooltip("destroy");
        $(this.name).replace(content.evalJSON().content);
        this.boxElement = $(this.name);
...
// }
/*
 * Used instead of onKeyPress because in Chrome it would not trigger when
 * hitting backspace or delete. With keyDown it works.
 */
FacetBox.prototype.onKeyDown = function(event) {
    switch (event.keyCode) {
    case Event.KEY_ESC:
        this.clear = true;
        break;
...
FacetBox.prototype.resetFilter = function() {
    // this is the filterField
    this.value = "type to filter";
    $(this).setStyle({
        "color" : "grey"
    });
}
...
FacetBox.prototype.onObserverEvent = function(event) {
    ...
    this.options.parameters = "filterToken=" + encodeURIComponent(this.filterField.value);
...
FacetBox.prototype.moveCursorToEnd = function() {
    var inputLength = this.filterField.value.length;
    this.filterField.focus();
    // focus() sets the cursor to the beginning of the input field for some
    // browsers. Set it to the end of the input.
...
FacetBox.prototype.selectAllFilterText = function() {
    var inputLength = this.value.length;
    this.focus();
    if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) {
        var textRange = this.createTextRange();
        textRange.move("character", 0), textRange.moveEnd("character", inputLength), textRange.select();
    } else
        this.setSelectionRange(0, inputLength);
}
