function facetSelectionDialog() {
	$j(function() {
		...
		$j("#facetSelectionList").bind("mousedown", function(e) {
			...
			unselected: function(event, ui) {
				// selects the ontoDiv that's in the check-box that was unselected'
				var $ontoDiv = $j(ui.unselected).children(".ontoDiv");
	...
	$j(function() {
		$j("#facetSelectionList > li").each(function(index) {
			...
			// mark it as selected.
			if(!$j("#facetSelectionList > li > input:eq("+index+")").is(":checked")) { //deleted ! before $
...
function styleFacets() {
	$j(document).tooltip({
		track: true
	});
	var ontoCheck = $j('.ontoDiv').parent();

	// assigns the name of the facet to the checkbox-title for the tooltips
	$j('.ontoDiv').each( function () {
		$j($j(this)).parent().attr('title',$j(this).text());
	});
	
	// solves the issue with "ui-dialog-content" of the facet-selection-list becomes scrollable
	// that was an unwanted side effect of a changed styling  
	$j($j("#facetSelectionList").parent()).css("overflow","hidden")
};