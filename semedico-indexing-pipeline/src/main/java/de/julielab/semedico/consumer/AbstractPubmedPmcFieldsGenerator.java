...
public abstract class AbstractPubmedPmcFieldsGenerator extends AbstractSemedicoFieldsGenerator {
	...
	protected void createAbstractSectionsField(JCas aJCas, Document doc)
			throws CASException, FieldGenerationException {
	...
	protected void createSentencesField(JCas aJCas, Document doc) throws CASException, FieldGenerationException {
		ArrayFieldValue sentences = getTextSpanFieldValues(aJCas, Sentence.type);
		doc.addField("sentences", sentences);
	}

	protected void createEventsField(JCas aJCas, Document doc) throws CASException, FieldGenerationException {
		FSIterator<Annotation> it = aJCas.getAnnotationIndex(FlattenedRelation.type).iterator();
		FieldValueGenerator eventGenerator = getInnerDocumentGenerator(EventInnerDocumentGenerator.class);
		ArrayFieldValue events = new ArrayFieldValue();
		while (it.hasNext()) {
			FlattenedRelation event = (FlattenedRelation) it.next();
			IFieldValue eventDoc = eventGenerator.generateFieldValue(event);
			events.add(eventDoc);
		}
