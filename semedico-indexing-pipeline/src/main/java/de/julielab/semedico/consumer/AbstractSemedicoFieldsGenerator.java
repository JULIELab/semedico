 ...
 * 
 */
public abstract class AbstractSemedicoFieldsGenerator extends FieldGenerator {
	...
	public AbstractSemedicoFieldsGenerator(FilterRegistry filterRegistry) {
		...
		fb = filterRegistry.getFilterBoard(SemedicoFilterBoard.class);
		...
		while (it.hasNext()) {
			...
			if (span.getBegin() < 0 || span.getEnd() > aJCas.getDocumentText().length()) {
				log.warn("Annotation of type {} occured with begin={} and end={} (document text length: {}). Ignoring",
						 span.getClass().getName(), span.getBegin(), span.getEnd(),
	...
	protected int getMeanLikelihood(AnnotationFS annotation) throws FieldGenerationException {
