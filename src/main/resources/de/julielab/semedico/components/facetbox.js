...
FacetBox.prototype.refreshListeners = function() {
	...
	if (this.prevButton
			&& this.prevButton.id == this.name + "PagerPreviousLink") {
		this.prevButtonListener = this.showPreviousBatch
	...
	if (this.expanded) {
		...
		if (this.filterField) {
			...
			Event.observe(this.filterField, "keydown", this.onKeyDown
					.bindAsEventListener(this));
			if (this.filterField.value == "type to filter")
				Event.observe(this.filterField, "focus", this.filterField.clear
						.bindAsEventListener(this.filterField));
			else
				this.filterField.focus();
		}
	}
	if (this.modeSwitchButtonListener)
		Event.stopObserving(this.modeSwitchButton, "click",
				this.modeSwitchButtonListener);

	this.modeSwitchButton = $(this.name + "ModeSwitchLink");
	if (this.modeSwitchButton) {
		this.modeSwitchButtonListener = this.toggleHierarchicMode
				.bindAsEventListener(this);
		Event.observe(this.modeSwitchButton, "click",
				this.modeSwitchButtonListener);
	}
	if (this.collapseButtonListener)
		Event.stopObserving(this.collapseButton, "click",
				this.collapseButtonListener);

	this.collapseButton = $(this.name + "CollapseLink");
	if (this.collapseButton) {
		this.collapseButtonListener = this.toggleCollapse
				.bindAsEventListener(this);
		Event
				.observe(this.collapseButton, "click",
						this.collapseButtonListener);
	}

	if (this.closeButtonListener)
		Event
				.stopObserving(this.closeButton, "click",
	...
	if (this.pathLinksListeners) {
		for ( var i = 0; i < this.pathLinksListeners.length; i++) {
			Event.stopObserving(this.pathLinks[i], "click",
					this.pathLinksListeners[i]);
		}
	}
	...
	while (true) {
		...
		if (link) {
			...
			this.pathLinksListeners[index] = this.drillUp
					.bindAsEventListener(this);
...
FacetBox.prototype.onListFiltered = function(request) {
	...
	if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) {
		...
		textRange.move("character", inputLength), textRange.moveEnd(
				"character", inputLength), textRange.select();
	} else
...
FacetBox.prototype.toggleHierarchicMode = function(event) {
	...
	this.options.parameters = "hierarchicMode="
			+ (this.hierarchicMode ? "false" : "true");
