function setupAutoCompleter(elementId, url, options) {
    ...
    options.resultsFormatter = function(item) {
        ...
        if (item['type'] == 'term') {
            ...
            // suggestions in the respective facet; I currently don't think this
    ...
    options.onAdd = function(item) {
        ...
        // unfortunately also leads the the repeated call of this method after
    ...
    options.remainingInputTokenizer = function(prefix) {
        var remainingTokens = [];
        // return the deferred object (similar concept to Futures in Java) from
        // the asynchronous call
        return $.getJSON(options.conceptRecognitionUrl + '?q=' + encodeURI(prefix));
    };
    options.onRemainingTokensInserted = function() {
        // This handler is called when multiple words are input and only after
        // that, a suggestion is selected. Then, all input before the accepted
        ...
        // After this is done, all new tokens must be handled according to their
...
//    };
    ...
    options.propertiesToSubmit = function() {
        return [ this.tokenValue, this.propertyToSearch, "facetid", "freetext", "tokentype", "lexertype", "termid",
                ...
                "userselected", "facetname", "preferredname" ]
    };

    // if we don't set this to false, clicking on the "find" button will delete
