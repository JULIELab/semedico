...
public interface ITermDatabaseService {
	...
	/**
	 * Only required in conjunction with addTerm to acutally commit the terms.
	 * 
	 * @deprecated This is more of a hack, it will be decided later in which exact way terms should be inserted into the
	 *             DB.
	 */
	@Deprecated
	 ...
	 * @see #getShortestPathFromAnyRoot(String, String, Type...)
	 */
	 ...
	 * relationship types specified by <tt>types</tt>. Internally, uses {@link #getPathsFromRoots(String, Type...)} to
	 ...
	 * @see #getPathsFromRoots(String, Type...)
	 */
	...
	/**
	 * <p>
	 * Returns the same data as {@link #getAllPathsFromAnyRoots(String, String, Type...)} but for a <tt>Collection</tt>
	 ...
	 *            The types to restrict the found path to.
	 * @return All paths from a facet root to the term with ID <tt>termId</tt>, sorted ascending by path length.
	 * @see #getAllPathsFromAnyRoots(String, String, Type...)
	 */
	...
	// public JSONArray getFacetRootTerms(String facetId);


	/**
	 * <p>
	 * Retrieves a number <tt>limit</tt> of terms from the database, beginning at <tt>skip+1</tt> with label
	 * <tt>label</tt>. The returned terms are unconnected, i.e. no relationships are loaded.
	 * </p>
	 * 
	 * @param label
	 * @param skip
	 ...
	 * @return
	 */
	// public JSONArray getUnconnectedTermsWithLabel(General label, int skip,
	// int limit);
