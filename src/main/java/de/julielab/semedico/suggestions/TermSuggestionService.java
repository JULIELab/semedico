...
public class TermSuggestionService implements Comparator<SuggestionHit>,
		ITermSuggestionService {

	private static final int MAX_SUGGESTION_LENGTH = 50;
	private static Logger logger = LoggerFactory
			.getLogger(TermSuggestionService.class);
	...
	public TermSuggestionService(
			@InjectService("SuggestionReader") IIndexReaderWrapper indexReaderWrapper,
		...
		try {
			if (directory == null) {
				Directory fileDirectory = indexReaderWrapper.getIndexReader()
						.directory();
	 ...
	 * 
	 * @see
	 * de.julielab.stemnet.suggestions.ITermSuggestionService#createSuggestions
	 * (java.lang.String, java.util.List)
	 */
	public List<FacetSuggestionHit> createSuggestions(String termFragment,
			List<Facet> facets) throws IOException {
		IndexSearcher searcher = new IndexSearcher(directory);
		QueryParser queryParser = new QueryParser(Version.LUCENE_23,
				Fields.SUGGESTION, new KeywordAnalyzer());
		...
		for (String word : words) {
			...
			// The Lucene parser will throw an error when a not-quoted term is
		...
		try {
			...
			for (Facet facet : facets) {
				...
				String query = termQuery + "+" + Fields.FACET + ":\""
						+ facet.getName() + "\"";
				TopScoreDocCollector collector = TopScoreDocCollector.create(
						MAX_SUGGESTION_LENGTH, true);
				...
				for (int i = 0; i < hits.length && resources.size() < 50; i++) {
					...
					SuggestionHit suggestionHit = new SuggestionHit(
							doc.get(Fields.SUGGESTION));

					suggestionHit.setIdentifier(identifier);
					suggestionHit.setIndex(index);
					suggestionHit.setShortDescription(doc
							.get(Fields.SHORT_DESCRIPTION));

					// termHit.setFrequency(new
					// Integer(doc.get(TermIndexer.Fields.FREQUENCY)));
	...
	public void createSuggestionIndex(String path) throws IOException,
		...
		IndexWriter writer = new IndexWriter(FSDirectory.open(new File(path)),
				new IndexWriterConfig(Version.LUCENE_23, analyzer));
		Collection<FacetTerm> terms = termService.filterTermsNotInIndex(termService
				.getNodes());
		int count = 0;
		for (FacetTerm term : terms) {
			...
			if (!(facetId.equals(Facet.FIRST_AUTHOR_FACET_ID) || facetId
					.equals(Facet.LAST_AUTHOR_FACET_ID)))
				termOccurrenceFilterService.filterTermOccurrences(term,
						occurrences);
			...
			for (String suggestion : suggestions)
				writer.addDocument(createIndexDocument(suggestion, facet, null,
						id, shortDescription));
	...
	private Document createIndexDocument(String term, String facet,
			String index, String indexedRepresentation, String shortDescription) {
		Document doc = new Document();

		Field termField = new Field(Fields.SUGGESTION, term, Field.Store.YES,
				Field.Index.ANALYZED);
		doc.add(termField);

		Field facetField = new Field(Fields.FACET, facet, Field.Store.YES,
				Field.Index.NOT_ANALYZED);
		doc.add(facetField);

		Field resourcesField = new Field(Fields.TERM_IDENTIFIER,
				indexedRepresentation, Field.Store.YES, Field.Index.NO);
		...
		if (shortDescription != null) {
			...
			Field descriptionField = new Field(Fields.SHORT_DESCRIPTION,
					filteredDescription, Field.Store.YES, Field.Index.NO);
	...
	class NGramAnalyzer extends Analyzer {
		...
		public TokenStream tokenStream(String fieldName, Reader reader) {
			if (maxTokenLength > 0)
				return new LowerCaseFilter(new EdgeNGramTokenFilter(
