...
//		javaScriptSupport.require("semedico/autocomplete").priority(InitializationPriority.EARLY).with(getClientId(), getCallbackURL(), parameters);
	...
	{
		CaptureResultCallback<List<FacetTermSuggestionStream>> callback = new CaptureResultCallback<List<FacetTermSuggestionStream>>();
	...
	private JSONArray toJSON(List<FacetTermSuggestionStream> facets)
	{
		...
		for (FacetTermSuggestionStream stream : facets)
		{
			...
			// facetObject.put("name", stream.getFacet().getName());
			// facetObject.put("type", "facet");
			// facetObject.put("termSize", stream.size());
			// array.put(facetObject);
			// }
			while (stream.incrementTermSuggestion())
			{
				...
				// if (!stream.getPreferredName().equals(stream.getTermName()))
